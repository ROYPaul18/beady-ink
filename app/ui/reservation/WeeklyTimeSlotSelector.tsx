...
import { addDays, format, startOfWeek, isBefore, isSameDay, parseISO, addMinutes } from "date-fns";
...
export default function WeeklyTimeSlotSelector({ salon, durationInMinutes, onSelect }: WeeklyTimeSlotSelectorProps) {
  const [startDate, setStartDate] = useState(startOfWeek(new Date(), { weekStartsOn: 1 }));
  const [weeklyTimeSlots, setWeeklyTimeSlots] = useState<{ [date: string]: string[] }>({});
  ...
  const fetchOpeningHoursAndSlots = async () => {
    ...
    try {
      const dates = daysOfWeek.map((_, i) => format(addDays(startDate, i), "yyyy-MM-dd"));
      const queryString = `salon=${encodeURIComponent(salon)}&dates=${dates.join(",")}`;

      const response = await fetch(`/api/opening-hours?${queryString}`);
      if (!response.ok) {
        throw new Error("Erreur lors de la récupération des créneaux et horaires d'ouverture");
      }
      ...
      dates.forEach((date) => {
        ...
        if (!dayInfo?.isClosed && dayInfo?.startTime && dayInfo?.endTime) {
          newTimeSlots[date] = generateTimeSlots(date, dayInfo.startTime, dayInfo.endTime);
        } else {
          newTimeSlots[date] = []; // Pas de créneaux si fermé
        }
      });
  ...
  const generateTimeSlots = (date: string, startTime: string, endTime: string): string[] => {
    const slots: string[] = [];
      ...
      <div className="flex justify-between items-center mb-4">
        <button
          onClick={() => {
            ...
            if (!isBefore(newDate, startOfWeek(new Date(), { weekStartsOn: 1 }))) {
              setStartDate(newDate);
            }
          }}
          className={`text-green font-bold text-lg ${isBefore(startDate, startOfWeek(new Date(), { weekStartsOn: 1 })) ? "text-gray-300 cursor-not-allowed" : ""}`}
          disabled={isBefore(startDate, startOfWeek(new Date(), { weekStartsOn: 1 }))}
        >
          &larr;
        </button>
        <h2 className="text-green text-xl font-bold">
          Semaine du {format(startDate, "dd MMM", { locale: fr })} au {format(addDays(startDate, 6), "dd MMM", { locale: fr })}
        </h2>
        <button onClick={() => setStartDate((date) => addDays(date, 7))} className="text-green font-bold text-lg">
          &rarr;
        </button>
      </div>
            ...
            return (
              <div key={day} className="flex flex-col items-center">
                <h3 className="font-semibold text-green mb-2 capitalize">
                  {day}
                  <span className="block text-sm text-gray-500">{displayDate}</span>
                </h3>
                <div className="space-y-1 w-full">
                  {isPastDay ? (
                    ...
                    slots.map((slot) => {
                      const isSelected = selectedSlot?.date === formattedDate && selectedSlot?.time === slot;
                      const endTime = format(addMinutes(parseISO(`${formattedDate}T${slot}`), effectiveDuration), "HH:mm");

                      return (
                        <button
                          key={`${formattedDate}-${slot}`}
                          onClick={() => handleSlotSelect(formattedDate, slot)}
                          className={`w-full p-2 text-sm rounded text-center transition-colors ${isSelected ? "bg-green text-white" : "bg-gray-100 hover:bg-gray-200 text-green"}`}
                        >
      ...
      {selectedSlot && (
        <div className="mt-4 p-4 bg-green/10 rounded">
          <p className="text-green text-center font-semibold">
            Créneau sélectionné : {format(new Date(selectedSlot.date), "dd/MM/yyyy", { locale: fr })} de{" "}
            {selectedSlot.time.replace(":", "h")} à{" "}
            {format(addMinutes(parseISO(`${selectedSlot.date}T${selectedSlot.time}`), effectiveDuration), "HH:mm").replace(":", "h")}
            <br />
            <span className="text-sm">Durée : {effectiveDuration} minutes</span>
          </p>
        </div>
      )}
    </div>
